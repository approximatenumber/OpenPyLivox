...
class _dataCaptureThread(object):
    ...
    def __init__(self, sensorIP, data_socket, imu_socket, filePathAndName, fileType, secsToWait, duration, firmwareType, showMessages, format_spaces, deviceType, convert_to_csv=True, logger=logging.getLogger('opl'), debug_data=False):
        ...
        if self.debug_data:
            logger.warning("Debug data is on!")
...
class openpylivox(object):
    ...
    def __init__(self, showMessages=False, logger=logging.getLogger('opl'), debug_data=False):
    ...
    def _dataStart_RT_B(self):
        ...
        if self._isConnected:
            if not self._isData:
                self._captureStream = _dataCaptureThread(self._sensorIP, self._dataSocket, self._imuSocket, "", 2, 0, 0, 0, self._showMessages, self._format_spaces, self._deviceType, logger=self.logger, debug_data=self.debug_data)
                time.sleep(0.12)
                self._waitForIdle()
                self._cmdSocket.sendto(self._CMD_DATA_START, (self._sensorIP, 65000))
                if self._showMessages: self.logger.info("   " + self._sensorIP + self._format_spaces + "   <--     sent start data stream request")
                    ...
                    if firmwareType >= 1 and firmwareType <= 3:
                        if dataType >= 0 and dataType <= 8:
                            ...
                            if firmwareType == 1 and dataType == 0:
                                ...
                                dataClass = 4
                                divisor = 18
                            elif firmwareType == 1 and (dataType == 2 or dataType == 7):
